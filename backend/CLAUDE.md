...
Stack: FastAPI + SQLAlchemy 2.0 (synchronous, deliberately) + Pydantic v2. SQLite for local dev, Postgres in production — the same code must work against both.
...
There is no pytest suite, linter, or build step — `smoke_test.sh` is the test. `rm WhereTheyAt.db` resets local state.
...
- **Schema changes**: no Alembic. `Base.metadata.create_all()` on startup only creates missing tables — it does not alter existing ones, so a column change needs the local DB deleted (or Alembic introduced, which is the intended upgrade path).
...
- **`ensure_columns()` in `main.py` is a mini-migration**: `create_all` never ALTERs existing tables, so at startup it bolts on columns added since a database was created (`users.clerk_id` + its unique index, `users.is_admin`, `events.starts_at`/`ends_at`). New columns must be added there as well as in `models.py`. Rows from before auth have `clerk_id NULL` and are unreachable from the app (fine for old local test users). Fold this into Alembic when it's introduced.
